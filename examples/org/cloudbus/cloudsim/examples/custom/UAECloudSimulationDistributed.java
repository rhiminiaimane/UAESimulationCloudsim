...
/**
 * Simulation CloudSim pour Université Abdelmalek Essaadi (UAE) - Distributed Datacenters
 */
public class UAECloudSimulationDistributed {
    ...
    public static void main(String[] args) {
        ...
        Log.printLine("SCÉNARIO DATACENTERS DISTRIBUÉS");
        Log.printLine("========================================");

        Log.printLine("\n\n" + "=".repeat(80));
        Log.printLine("DÉBUT DU SCÉNARIO 2 : DATACENTERS DISTRIBUÉS");
        Log.printLine("=".repeat(80));

        ScenarioResult result = runScenario(2);
        if (result == null) {
            Log.printLine("Erreur: Le scénario distribué n'a pas pu être exécuté.");
        }

        Log.printLine("\n" + "=".repeat(80));
        Log.printLine("FIN DU SCÉNARIO 2 : DATACENTERS DISTRIBUÉS");
        Log.printLine("=".repeat(80));
    }
    ...
    private static ScenarioResult runScenario(int scenario) {
        try {
            ...
            // Create distributed datacenters - one per campus
            globalDatacenters = new ArrayList<>();
            Log.printLine("Création des datacenters distribués...");
            globalDatacenters.add(createDatacenter("DC_Tetouan", 50, 8, 262144, 2000, 100, 400));
            globalDatacenters.add(createDatacenter("DC_Tanger", 40, 8, 196608, 2500, 80, 300));
            globalDatacenters.add(createDatacenter("DC_Hoceima", 30, 6, 131072, 1500, 70, 250));
            globalDatacenters.add(createDatacenter("DC_Larache", 20, 4, 65536, 1500, 50, 200));
            globalDatacenters.add(createDatacenter("DC_KsarElKebir", 20, 4, 65536, 1600, 60, 220));
            ...
            // Create VMs for distributed datacenters (each campus hosts its own VMs)
            Log.printLine("\n" + "-".repeat(50));
            Log.printLine("CRÉATION DES VMs - DATACENTERS DISTRIBUÉS");
            Log.printLine("-".repeat(50));
            Log.printLine("🎯 Stratégie: VMs distribuées sur chaque datacenter de campus");

		createVMsForCampus(globalVmList, brokerTetouanId, 0, 70, 1500, 2048, 2, 8000, 800, "LMS_Tetouan");
		createVMsForCampus(globalVmList, brokerTetouanId, 70, 50, 2000, 4096, 2, 10000, 1000, "Sciences_Tetouan");
		createVMsForCampus(globalVmList, brokerTangerId, 120, 60, 2500, 8192, 4, 15000, 1500, "HPC_Tanger");
		createVMsForCampus(globalVmList, brokerTangerId, 180, 40, 2000, 4096, 2, 9000, 900, "Medecine_Tanger");
		createVMsForCampus(globalVmList, brokerHoceimaId, 220, 30, 1500, 2048, 2, 5000, 500, "Edge_Hoceima");
		createVMsForCampus(globalVmList, brokerLaracheId, 250, 25, 1500, 2048, 2, 6000, 600, "FP_Larache");
		createVMsForCampus(globalVmList, brokerKsarElKebirId, 275, 25, 1600, 2048, 2, 7000, 600, "FP2_KsarElKebir");
            ...
            // Create cloudlets for distributed datacenters
		createCloudletsForCampus(cloudletList, brokerTetouanId, 0, 400, 40000, 500, 300, 2,
			brokerTetouan.getVmList(), "LMS_Tetouan");
		createCloudletsForCampus(cloudletList, brokerTetouanId, 400, 300, 100000, 1024, 512, 2,
			brokerTetouan.getVmList(), "Sciences_Tetouan");
		createCloudletsForCampus(cloudletList, brokerTangerId, 700, 300, 500000, 5120, 2048, 8,
			brokerTanger.getVmList(), "HPC_Tanger");
		createCloudletsForCampus(cloudletList, brokerTangerId, 1000, 150, 200000, 10240, 5120, 4,
			brokerTanger.getVmList(), "Medical_Tanger");
		createCloudletsForCampus(cloudletList, brokerHoceimaId, 1150, 100, 30000, 400, 200, 2,
			brokerHoceima.getVmList(), "Edge_Hoceima");
		createCloudletsForCampus(cloudletList, brokerLaracheId, 1250, 100, 25000, 400, 200, 2,
			brokerLarache.getVmList(), "FP_Larache");
		createCloudletsForCampus(cloudletList, brokerKsarElKebirId, 1350, 50, 35000, 450, 250, 2,
			brokerKsarElKebir.getVmList(), "FP_KsarElKebir");
            ...
            Log.printLine("\n" + "=".repeat(50));
            Log.printLine("DÉMARRAGE SIMULATION - DATACENTERS DISTRIBUÉS");
            Log.printLine("=".repeat(50));
    ...
    private static void printDatacenterDetails(int scenario) {
        Log.printLine("\n" + "-".repeat(60));
        Log.printLine("CONFIGURATION DES DATACENTERS - SCÉNARIO DISTRIBUÉ");
    ...
    private static ScenarioResult collectDetailedScenarioResults(int scenario, DatacenterBroker... brokers) {
        ScenarioResult result = new ScenarioResult(scenario, "DATACENTERS DISTRIBUÉS");
